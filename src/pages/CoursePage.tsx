...
type ClassType = "all" | "live" | "video"; // ✅ FIX: เอา "\n" ออก
...
export default function CoursesPage() {
  ...
  async function startCheckout(courseId: string, priceId: string) {
    ...
    // ✅ MOCK MODE: ทำเหมือนจริง -> ไปหน้า SuccessfullyBuy
    if (usingMock) {
      window.setTimeout(() => {
        setBusy(null);
        nav(
          `/SuccessfullyBuy?mock=1&courseId=${encodeURIComponent(courseId)}`,
          { replace: true },
        );
      }, 400);
      return;
    ...
    try {
      const { url } = await fetchWithAuth<{ url: string }>(
        `${API_BASE}/api/checkout/create-session`,
        {
          courseId,
          items: [{ qty: 1, priceId }],
          successUrl: `${window.location.origin}/SuccessfullyBuy?courseId=${encodeURIComponent(
            courseId,
          )}`,
          cancelUrl: `${window.location.origin}/ErorrPage?courseId=${encodeURIComponent(
            courseId,
          )}`,
        },
      );
      ...
      console.error(e);
      setBusy(null);
      // ✅ fallback ไป error page ให้ consistent
