...
import type { CoursePageResponse, CoursePageUser } from "./CourseType.ts";
...
export default function CoursesPage() {
  ...
  async function startCheckout(courseId: string, priceId: string) {
    ...
    // ✅ mock mode -> redirect ไปหน้า success จริง
    if (usingMock) {
      setTimeout(() => {
        nav(
          `/SuccessfullyBuy?courseId=${encodeURIComponent(courseId)}&mock=1`,
          {
            replace: true,
          },
        );
    ...
    try {
      const { url } = await fetchWithAuth<{ url: string }>(
        `${API_BASE}/api/checkout/create-session`,
        {
          courseId,
          items: [{ qty: 1, priceId }],
          successUrl: `${window.location.origin}/SuccessfullyBuy?courseId=${encodeURIComponent(courseId)}&session_id={CHECKOUT_SESSION_ID}`,
          cancelUrl: `${window.location.origin}/ErorrPage?courseId=${encodeURIComponent(courseId)}`,
        },
      );
      ...
      console.error(e);
      nav(`/ErorrPage?courseId=${encodeURIComponent(courseId)}`, {
        replace: true,
      });
      setBusy(null);
    }
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        {(["BEST_SELLER", "TRENDING", "NEW"] as BadgeType[]).map((b) => (
          <button
            ...
            className={`px-3 py-2 rounded-xl border border-black ${
              badgeType.includes(b) ?
                "bg-black text-white"
              : "bg-white text-black"
            }`}
          >
